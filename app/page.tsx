...
const projects = [
  ...
  },
  {
    ...
    cases: [
      {
        title: "N+1 조회 개선으로 요약 카드 응답 성능 최적화",
        category: "Performance",
        problem:
          "그룹 멤버의 닉네임을 조회할 때 멤버를 개별 로딩해 멤버 50명 기준 51개의 쿼리가 발생하는 문제를 발견했습니다.",
        solution:
          "반복 조회되는 Repository에 @EntityGraph를 적용해 연관 Entity를 단일 쿼리로 조회하도록 변경했습니다.",
        result:
          "멤버 50명 기준 쿼리 수를 51개에서 1개로 줄이고, 응답 시간을 10,280ms에서 229ms로 45배 단축했습니다. 벤치마크 테스트로 멤버 수별 개선 효과를 검증했습니다.",
        retrospective:
          "쿼리 수와 응답 시간은 줄였지만 멤버 50명까지의 벤치마크에 머물렀습니다. 실제 운영에서는 데이터가 더 많아질 때도 같은 효과가 유지되는지 확인하고, 필요한 연관 데이터만 조회하는지도 함께 검증할 필요가 있습니다.",
      },
      {
        title: "비용 구조를 고려한 영수증 인식 기술 선정",
        category: "AI Integration",
        problem:
          "고정요금 OCR은 월 18,000원이 발생했고, 멀티모달 모델은 응답 형식이 일정하지 않아 필드 누락과 토큰 낭비가 발생했습니다.",
        solution:
          "Gemini API를 선택하고 미인식 값은 null, 응답은 고정 JSON만 반환하도록 프롬프트를 설계하고 응답 검증 로직을 구현했습니다.",
        result:
          "비용을 건당 약 0.8원으로 전환하고, 누락 필드와 응답 형식을 일관되게 처리했습니다.",
        retrospective:
          "비용은 확인했지만 다양한 영수증 유형에 대한 정확도 지표는 아직 없습니다. 실패 유형별 테스트셋과 필드별 정확도 측정이 필요합니다.",
      },
      {
        title: "민감정보 보호를 위한 계좌번호 암호화",
        category: "Security",
        problem:
          "정산 계좌번호를 평문으로 저장하면 DB 노출 시 민감정보가 그대로 유출될 수 있었습니다.",
        solution:
          "AES-256-GCM으로 암호화하고 레코드마다 새 IV를 생성했습니다. 암호화 키는 환경 변수로 주입해 코드와 분리했습니다.",
        result:
          "DB에는 암호문과 IV만 저장해 데이터 조회만으로 계좌번호 원문을 확인할 수 없게 했습니다.",
        retrospective:
